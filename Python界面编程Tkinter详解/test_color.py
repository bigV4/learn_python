...
for color in colors.split('\n'):
    sp = color.split(' ')
    try:
        tk.Label(text=color, bg=sp[1]).grid(
            row=int(i/colcut), column=i % colcut, sticky=tk.W+tk.E+tk.N+tk.S)
    except Exception as e:
        print('err', color)
        tk.Label(text='ERR'+color).grid(row=int(i/colcut),
                                        column=i % colcut, sticky=tk.W+tk.E+tk.N+tk.S)
    i += 1
root.mainloop()
